...
modbus_address_path=config.MODBUS_ADDRESS_PATH
...
async def main():
    ...
    try:
        async with await CameraCtrlForShuttle.create(
                ...
                modbus_address_path=modbus_address_path,
                **redis_con,
                **modbus_con,
        ) as camera_ctrl:
            # 等待所有任务运行
        ...
        _logger.warning(f"[Main] cameras server for shuttle cancelled")
    except Exception as err:
        _logger.exception(f"[Main] cameras server for shuttle error: {err}")
    finally:
        stop_event.set()
        _logger.info(f"[Main] cameras server for shuttle ended")
